    ...
    public function generate(GenerateRequest $request,OrderMessageService $orderMessageService) {
        try {
            ...
            if(LaravelGmail::check()) {
                ...
                info("Me: {$loggedEmail} And  {$fromEmailAddress} ");

                $messages = $orderMessageService->getMessages($order);

                // $sessionLab->setMessages($messages);

                // Get the total number of results
                $resultSizeEstimate = $messages['resultSizeEstimate']; // 450

                info("Messages Found : ".count($messages["messages"]));
                info("Total Found : ".$resultSizeEstimate);
                ...
                if($total_message_count > 0) {
                    // CREATE PDF
                    //TODO::PDF Creating Stopped for a while
                    // (new PdfService())
                    //     ->createPDF($messages, $email_address, $language, $inc_array);
                    ...
                    // session()->put('total_message', $total_message_count);
...
//                        'complete_message' => "Message Not Found: Please Try Again",
//                        'download_file' => null
//                    ], 200);
//                }
            }
