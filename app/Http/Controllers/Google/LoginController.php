...
{
    public function generateAgain(GenerateRequest $request, OrderMessageService $orderMessageService)
    {
        try {

            $data = $request->validated();
            $data["platform_type"] = 1;
            $data['timezone'] = geoip(request()->ip())->timezone;
            $data['search_keyword_list']=$request->search_keyword_list;

            if (session()->has('email_not_match')) {
                session()->remove('email_not_match');
                session()->save();
            ...
            if (LaravelGmail::check()) {
                ...
                if (isset($redisData['pdf_token']) && $redisData['pdf_token']['email'] == $request->your_email){
                    $token = $redisData['pdf_token']['access_token'];
                }else if(isset($redisData['main_token']) && $redisData['main_token']['email'] == $request->your_email) {
                ...
                $search_attachments_list    = $request->search_attachments_list;
                ...
                //Set emails in the session
                $fromEmailAddress = $request->email_from;
                $sessionLab->setEmailFrom($fromEmailAddress);
                $sessionLab->setYourEmail($request->your_email);

                $order = $orderMessageService->storeOrder($data);

                $messages = $orderMessageService->getMessages($order, false,$token);

                $total = count($messages);
                info('Total messages in the Generate New' . $total);
    ...
    public function generate(GenerateRequest $request, OrderMessageService $orderMessageService)
    {
        try {
            ...
            $data = $request->validated();
            $data["platform_type"] = 1;
            $data["search_attachments_list"] =  $request->search_attachments_list;
            $data['timezone'] = geoip(request()->ip())->timezone;
            $data['search_keyword_list']=$request->search_keyword_list;
            ...
            $redisData = getSessionDataFromRedis();
            if (LaravelGmail::check()) {
                if (isset($redisData['pdf_token']) && isset($redisData['pdf_token']['email']) && $redisData['pdf_token']['email'] == $request->your_email){
                    $token = $redisData['pdf_token']['access_token'];
                }else if(isset($redisData['main_token']) && isset($redisData['main_token']['email']) && $redisData['main_token']['email'] == $request->your_email) {
                    $token = $redisData['main_token']['access_token'];
                }
                ...
                if ($inc_keywords) {
                    $inc_array = array_map('trim', explode(',', $inc_keywords));
                    $inc_query = $this->createRawIncludeQuery($inc_array);
                }

                if ($exc_keywords) {
                    $exc_array = array_map('trim', explode(',', $exc_keywords));
                    $exc_query = $this->createRawExcludeQuery($exc_array);
                }
...
<h3 class="title">- Page {PAGENO} of {nbpg} -</h3>
    ...
    protected function createRawIncludeQuery($array)
    {
        ...
        if (!empty($array)) {
            $quotedArray = array_map(fn($keyword) => '"' . trim($keyword) . '"', $array);
            $string = implode(" OR ", $quotedArray);
        }
