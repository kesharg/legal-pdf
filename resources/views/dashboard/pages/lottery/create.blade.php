...
@section('title', localize(' - Create Lottery'))
@push('extra-styles')
...
<div class="row">
    <div class="col-md-12 col-lg-12 col-sm-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">{{localize('Create New Lottery')}}</h4>
                <br>
                {{-- <p class="card-description"> From here you can create new lottery. </p> --}}
                <form class="forms-sample" action="{{ route('distributor.lottery.create') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group has-validation">
                        <label for="title">{{localize('Title')}}</label>
                        <span style="color: red">*</span>
                        <input type="text" class="form-control @error('title') is-invalid @enderror" value="@if(isset($lottery)) {{ $lottery->title }} @endif" id="title" name="title" placeholder="eg: Boxing Day Lottery">
                        @error('title')
                    ...
                    <div class="form-group has-validation">
                        <label for="brand">{{localize('Series')}}</label>
                        <select name="series_id" id="series_id" class="form-control select2 @error('series_id') is-invalid @enderror" style="padding: 0.94rem 1.375rem!important;">
                            <option value="">{{localize('Choose Series')}}</option>
                            @foreach($series as $srs)
                                <option value="{{ $srs->id }}" @if(isset($lottery)) @if ($srs->id == $lottery->series_id) selected @endif @endif>{{ $srs->model?->model_name }} || {{ $srs->guantity }}|| Serial No {{ $srs->starting_serial_no }} - {{ $srs->ending_serial_no }}  </option>
                            @endforeach
                        </select>
                        @error('series_id')
                    ...
                    <button type="submit" class="btn btn-gradient-primary me-2">{{localize('Create')}}</button>
